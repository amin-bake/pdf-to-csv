...
export default function PdfToTextPage() {
  ...
  const handleDownloadFile = async (fileId: string) => {
    ...
    try {
      await api.download(file.fileId, file.fileName.replace(".pdf", ".txt"));
    } catch (err) {
    ...
    try {
      if (completedFiles.length === 1) {
        ...
        const fileNames = completedFiles.reduce((acc, f) => {
          acc[f.fileId] = f.fileName.replace(".pdf", ".txt");
          return acc;
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="mb-8">
        ...
        <div className="text-center">
          <h1 className="text-4xl font-bold mb-2">PDF to Text Converter</h1>
          <p className="text-muted-foreground">
            Upload PDF files and extract plain text content
          </p>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <label htmlFor="merge" className="text-sm font-medium">
                Merge all content into single text file
              </label>
            </div>
          ...
          <CardContent>
            {currentJobId && conversionStatus ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                {hasFiles
                  ? 'Click "Convert to Text" to start processing'
